...
int solution(int current_grade[], int current_grade_len, int last_grade[], int last_grade_len) {
    ...
    int* rank = func_b(current_grade, arr_length);//1등 구하기
    int max_diff_grade = func_c(current_grade, last_grade, arr_length);//점수차가 가장 많은 사람 구하기
    int answer = func_a(current_grade, last_grade, rank, arr_length, max_diff_grade);//장학생 수 구하기
    return answer;
}
